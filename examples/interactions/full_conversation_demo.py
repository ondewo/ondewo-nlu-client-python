...
def get_response_from_request(session: str, text: str, context: Optional[Context] = None) -> DetectIntentResponse:
...
def make_context() -> context_pb2.Context:
    ...
    context: context_pb2.Context = context_pb2.Context(
        name=f"{session}/contexts/exact_intent", lifespan_count=20, parameters=parameter
    )
